...
lane_laneset_pair_t RoutedDriver::chooseNextLane_Laneset(Car *theCar)
{
	 ...
	 * and activate communication
	 *
	 */
	if ( viewCost.compare("LOCAL") == 0 && linksPath.size() % 2 == 0 ) {	// view cost local and linksPath.size is even (to reduce number of queries)

		graph.updateLocalCost(currentLink, this->getId()); // update local cost (own perception)

		if (communicate && linksPath.size() % 3 == 0) {
			graph.queryCost(currentLaneSet, linksPath, this->getId()); // driver ask for information
		}
	}
	...
	// graph.printLocalCost(this->getId());
	...
	if (replanner && activateReplanStrategy(currentLaneSet) && lastLanesetReplan != currentLaneSet->getId())
	{
		...
		//JB:: Replanning must be here
		nSrce = currentLink->gNodeZ;
		graph.ModifiedLinks.clear();
//		graph.updateLocalDensityCosts(nSrce, 2); // look two links ahead
		...
		if ( viewCost.compare("GLOBAL") == 0 ) {
			...
			graph.updateGraphDensityWithLocalCosts(this->getId()); // use local costs
		}
