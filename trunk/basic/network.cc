...
void Network::drawMap() {
	...
	for (itlst = lst.begin(); itlst!= lst.end(); itlst++) {
		...
		//cout << "text(" << (*itlst)->getEndNode()->getXcoord() << "," << (*itlst)->getEndNode()->getYcoord() << ",'" << (*itlst)->getEndNode()->getId() << "')"  << endl;
	}
}

/*
 * network occupation map inside MATLAB
 */
void Network::makeMap(MYSQL *connect, int odsize, string networkname, string round, int replan, int comm, int view) {
	//	double minx1 = 999999;
	//	double minx2 = 999999;
	//	double miny1 = 999999;
	//	double miny2 = 999999;
	...
	//	double maxy2 = 0;
		...
		double mapa_soma[200][200];
		int mapa_cont[200][200];
		int mapa_id[200][200];
		...
		map<int, double> m;
			...
			for (itlst = lst.begin(); itlst!= lst.end(); itlst++)
			{
				   m[(*itlst)->getId()] = median((*itlst)->vecDensity);

		//		if (minx1 > (*itlst)->getStartNode()->getXcoord()) {
		//			minx1 = (*itlst)->getStartNode()->getXcoord();
		//		}
		//		if (minx2 > (*itlst)->getEndNode()->getXcoord()) {
		//			minx2 = (*itlst)->getEndNode()->getXcoord();
		//		}
		//
		//		if (miny1 > (*itlst)->getStartNode()->getYcoord()) {
		//			miny1 = (*itlst)->getStartNode()->getYcoord();
		//		}
		//		if (miny2 > (*itlst)->getEndNode()->getYcoord()) {
		//			miny2 = (*itlst)->getEndNode()->getYcoord();
		//		}
		//
		//		if (maxx1 < (*itlst)->getStartNode()->getXcoord()) {
		//			maxx1 = (*itlst)->getStartNode()->getXcoord();
		//		}
		//		if (maxx2 < (*itlst)->getEndNode()->getXcoord()) {
		//			maxx2 = (*itlst)->getEndNode()->getXcoord();
		//		}
		//
		//		if (maxy1 < (*itlst)->getStartNode()->getYcoord()) {
		//			maxy1 = (*itlst)->getStartNode()->getYcoord();
		//		}
		//		if (maxy2 < (*itlst)->getEndNode()->getYcoord()) {
		//			maxy2 = (*itlst)->getEndNode()->getYcoord();
		//		}

	//			cout << (*itlst)->getId() << " - " << (*itlst)->getStartNode()->getXcoord() << "," << (*itlst)->getStartNode()->getYcoord() << " - "
	//					  << (*itlst)->getEndNode()->getXcoord() << "," << (*itlst)->getEndNode()->getYcoord() << endl;
				int xinicial, xfinal, yinicial, yfinal = 0;

				if ((*itlst)->getEndNode()->getXcoord()>(*itlst)->getStartNode()->getXcoord()){
					xfinal = (*itlst)->getEndNode()->getXcoord();
					xinicial = (*itlst)->getStartNode()->getXcoord();
				} else {
					xfinal = (*itlst)->getStartNode()->getXcoord();
					xinicial = (*itlst)->getEndNode()->getXcoord();
				}
				if ((*itlst)->getEndNode()->getYcoord()>(*itlst)->getStartNode()->getYcoord()){
					yfinal = (*itlst)->getEndNode()->getYcoord();
					yinicial = (*itlst)->getStartNode()->getYcoord();
				} else {
					yfinal = (*itlst)->getStartNode()->getYcoord();
					yinicial = (*itlst)->getEndNode()->getYcoord();
				}


				for (int ix=1;ix<=aprox;ix++) {
					for (int iy=1;iy<=aprox;iy++) {
				//		cout << i << endl;
		//				int ax2 = (*itlst)->getEndNode()->getXcoord()/10*i;
		//				int ax1 = ax2 - ((*itlst)->getEndNode()->getXcoord()/10);
		//				int ay2 = (*itlst)->getEndNode()->getYcoord()/10*i;
		//				int ay1 = ay2 - ((*itlst)->getEndNode()->getYcoord()/10);

						int ax = xfinal-(xfinal-xinicial)/ix;

						for (int i_=1;i_<=tilesize;i_++) {
						x2 = x2_max/tilesize*i_;
						x1 = x2 - (x2_max/tilesize);

						for (int i=1;i<=tilesize;i++) {
							y2 = y2_max/tilesize*i;
							y1 = y2 - (y2_max/tilesize);

						int ay = yfinal-(yfinal-yinicial)/iy;
		//				int ay = yinicial-(yinicial-yfinal)/ix;


	//					if ((*itlst)->getId() == 140) {
	//						cout << "ax=" << ax << " x1=" << x1 << " x2=" << x2 << "	-	";
	//						cout << "ay=" << ay << " y1=" << y1 << " y2=" << y2 << endl;
	//					}
	...
	//							if ((*itlst)->getId() == 140) {
	//								//140 - 10069.1, 2032.38 - 7227.8, 2383.52
	//	//							ax=9122
	//	//							ay=2266
	//								//cout << "id=" << (*itlst)->getId() << " (" << ax << "," << ay << ")" << " dentro do quadrante " << x1 << "," << x2 << " | " << y1 << "," << y2 << endl;
	//								cout << (x1+x2)/2 << " " << (y1+y2) << " 0";
	//							}

								//cout << (x1+x2)/2 << " " << (y1+y2) << " 0" << endl;

								//cout << "a" << i_ << " " << i << " " << m[(*itlst)->getId()] << endl;

								mapa_soma[i_][i] += m[(*itlst)->getId()];
								mapa_cont[i_][i] += 1;
								mapa_id[i_][i] += m[(*itlst)->getId()];
...
//				if (mapa_soma[i_][i]>0) {
//					cout << "d" << mapa_soma[i_][i] << " " << mapa_cont[i_][i] << endl;
//					cout << (x1+x2)/2 <<  " " << (y1+y2)/2 << " " << mapa_soma[i_][i] << endl;
//				}

				mapa_soma[i_][i] /= mapa_cont[i_][i];
				//cout << (x1+x2)/2 << " " << (y1+y2) << " " << (rand() % 10) << endl;
//				cout << mapa_id[i_][i] << " " << (x1+x2)/2 <<  " " << (y1+y2)/2 << " " << mapa_soma[i_][i] << endl;
				...
				//odsize, string networkname, string round, int replan, int comm, int view
				stringstream query;
				query << "insert into net_ocp (laneset_id, x, y, z, odsize, network, replan, comm, global, round) values (";
				query << mapa_id[i_][i] << "," << (x1+x2)/2 << "," << (y1+y2)/2 << "," << nz << ",";
				query << odsize << ",\"" << networkname << "\"," << replan << "," << comm << "," << view << "," << round << ")";
	...
	//	cout << "MINX:" << minx1 << "," << minx2 << " | MINY:" << miny1 << "," << miny2 << " | MAXX:" << maxx1 << "," << maxx2 << " | MAXY:" << maxy1 << "," << maxy2 << endl;


		//10069.1,2032.38 - 7227.8,2383.52
		//cout << sqrt(pow(xb-xa,2) + pow(yb-ya,2)) << endl;

		//dividir o maior x por 100
		//dividir o maior y por 100

			//dividir o x e y da aresta por 10 (em 10 partes)
				//comparar cada divisao da aresta com os tailings
