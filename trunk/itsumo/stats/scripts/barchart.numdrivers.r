...
matrix <- read.table('../poau/bar.numdrivers.data', header=TRUE)
err <- read.table('../poau/bar.numdrivers.error', header=TRUE)

postscript("../poau/bar.numdrivers.eps",
		width = 8.0, height = 5.0,
				  horizontal = FALSE, onefile = FALSE, paper = "special")

barx <- barplot(
	as.matrix(matrix), main="Average Number of Drivers", 
	angle = 15+10*1:5, ylab= "steps", ylim= c(0,26000), xlab="min travel time", beside=TRUE, 
