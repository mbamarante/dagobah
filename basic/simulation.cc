...
//					countCarsDiff = lastCars - active_cars.size();
//					lastCars = active_cars.size();
//
//					if ( countCarsDiff < 50 && countCarsDiff > 0	// only 50 cars left && the number reduce
//							&& active_cars.size() > 50 // avoid finishing simulation
//							&& currentStep > 100	// avoid first test
//							&& reduceOccupationSize == false ) {	// only once
//						cout << "\t50 triggered!" << endl;
//						reduceOccupationSize = true;
//					} // else {
//						// reduceOccupationSize = false;
//					// }
				...
				while (i < size) {
					...
					if (currentStep % 300 == 0) { // a cada 10 steps checa se o driver está parado
...
//							cout << "working... " << (*i) -> getDriver()->getId() << " ccell=" << (*i) -> getCurrentCell() << " lcell=" << (*i) -> getLastCell() << " step=" << currentStep << endl;
						} else {
				...
				// TENTATIVA DESESPERADA: VEÍCULO SAI E JÁ VOLTA PRA SIMULAÇÃO! MANTER A REDE OCUPADA!
				// esta sendo feito no driver loader logo que o driver registra sua saida.
//				for (vector<DriverLoader *>::iterator dit = vet_drivers.begin(); dit
//						!= vet_drivers.end(); dit++) {
